import { NextRequest, NextResponse } from "next/server";
import { prisma } from "@/lib/prisma";

// POST /api/share-click - logs a "share result" click event (public, fire-and-forget)
export async function POST(req: NextRequest) {
  try {
    const body = await req.json().catch(() => ({}));
    const visitorId = typeof body.visitorId === "string" ? body.visitorId.slice(0, 100) : null;
    const resultSlug = typeof body.resultSlug === "string" ? body.resultSlug.slice(0, 100) : null;
    const method = typeof body.method === "string" ? body.method.slice(0, 40) : "unknown";

    if (!visitorId) {
      return NextResponse.json({ error: "visitorId is required." }, { status: 400 });
    }

    await prisma.shareClickEvent.create({
      data: { visitorId, resultSlug, method },
    });

    return NextResponse.json({ success: true }, { status: 201 });
  } catch (error) {
    console.error("Error logging share click:", error);
    return NextResponse.json({ error: "Failed to log share click." }, { status: 500 });
  }
}
